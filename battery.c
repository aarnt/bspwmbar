...
battery_draw(battery_t *bat, draw_context_t *dc, module_option_t *opts)
{
	...
	color_t *fg;
	if (bat->status == BAT_CHARGING)
	{
          if (opts->battery.fg_charging)
            fg = color_load(opts->battery.fg_charging);
	  else 
	    fg = color_default_fg();
	}
	else if (bat->capacity < 30)
	{
	  if (opts->battery.fg_low_capacity)
	    fg = color_load(opts->battery.fg_low_capacity);
	  else
	    fg = color_default_fg();
	}
	else
	{
	  if (opts->battery.fg_high_capacity)
	    fg = color_load(opts->battery.fg_high_capacity);
	  else
	    fg = color_default_fg();
	}
		
	draw_color_text(dc, fg, buf);
}
